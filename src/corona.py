...
Class for exchanging data to Solr 
...
class corona:
	# to-do load from settings file 
	solrServer = 'http://localhost:8983/solr/kinda-sorta'
	inst = None
	def __init__(self): 
		pass
	...
	"""
	Bulldozed version of what I'd like to encapsulate in a RH or custom Solr code. Adjust the boostings weights and minimum to match dynamically. 
	...
	def ks_query(self, id, mq, oq, mat_weight, objName_weight, mat_mm, objName_mm): 
		url = self.solrServer + '/query?q='+ urllib.quote_plus('material:'+ mq + ' AND objectName:' + oq) \
		+ '&qf=material^' + str(mat_weight) \
		+ '&qf=objectName^' + str(objName_weight) \
		+ '&wt=json&indent=true'
